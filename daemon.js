...
const SOCK_FILE = path.join(IRIS_DIR, 'daemon.sock')
...
function startDaemon () {
  ...
  function resolveApiUrl () {
    ...
    if (fileConfig.iris_api_url && !/local\.|localhost|127\.0\.0\.1/.test(fileConfig.iris_api_url)) {
      return fileConfig.iris_api_url
    }
  ...
  acquireSocketLock(() => {
    ...
    function cleanup (signal) {
      ipcServer.close()
      try { fs.unlinkSync(SOCK_FILE) } catch { /* fine */ }
